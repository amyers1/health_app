...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, unit, icon, colorClass, subtitle, trend }) => {
  return (
    <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-slate-700 transition-colors">
      ...
      <div>
        ...
        {subtitle && <p className="text-slate-500 text-[10px] mt-1 truncate">{subtitle}</p>}
      </div>
    </div>
  );
};
